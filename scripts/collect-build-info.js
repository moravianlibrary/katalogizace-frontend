...
try {
  ...
  commitTag = execSync("git describe --tags --abbrev=0 --always")
    .toString()
    .trim();
  if (commitTag === commitHash) {
    commitTag = undefined; // If the tag is the same as the commit hash, set it to undefined
    console.log(
      "[collect-build-info] ⚠️ No tags found. Commit Tag is undefined.",
    );
  } else {
...
console.log(
  "[collect-build-info] ✅ Build info JSON generated, saving to file " +
    fileName,
);
