...
if (!fs.existsSync(devEnvPath)) {
  console.log(
    "[env-bootstrap] ⚠️ environment.local.ts not found. Creating it from environment.ts...",
  );
  ...
  try {
    ...
    console.log(
      "[env-bootstrap] ✅ environment.local.ts created successfully.",
    );
  } catch (error) {
  ...
  if (baseTime && devTime && baseTime > devTime) {
    console.warn(
      "[env-bootstrap] ⚠️ Warning: environment.ts is newer than environment.local.ts. Consider syncing or updating your local dev file if needed.",
    );
  }
}
