...
@Component({
  ...
  imports: [InputDropdownComponent, TranslateModule],
...
export class Field300EditorComponent {
  ...
  readonly indicators = computed(() => getIndicators('300'));
  ...
  private readonly firstInput =
    viewChild<ElementRef<HTMLInputElement>>('val_a');
  ...
  getSubfield(code: 'a' | 'b' | 'c' | 'e' | '3'): string {
    const f = this.field();
    if (!f) return '';
    return f.subfields?.find((s) => s.code === code)?.value ?? '';
  }

  setSubfield(code: 'a' | 'b' | 'c' | 'e' | '3', value: string) {
    const f = this.field();
    if (!f) return;
